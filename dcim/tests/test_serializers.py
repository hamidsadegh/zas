...
def _make_site(name, organization):
    return Site.objects.create(name=name, organization=organization)


def _make_area(name, site):
    return Area.objects.create(name=name, site=site)
...
def test_device_serializer_rack_queryset_uses_request_data():
    ...
    site = _make_site("Site One", org)
    area_a = _make_area("Area A", site)
    area_b = _make_area("Area B", site)
...
def test_device_serializer_rack_queryset_uses_instance_area():
    ...
    site = _make_site("Site Two", org)
    area_a = _make_area("Area A", site)
    _make_area("Area B", site)
...
def test_device_serializer_validate_rejects_rack_from_other_area():
    ...
    site = _make_site("Site Three", org)
    area_a = _make_area("Area A", site)
    area_b = _make_area("Area B", site)
    ...
    with pytest.raises(serializers.ValidationError) as excinfo:
        serializer.validate({"site": site, "area": area_a, "rack": rack_b})

    assert "Rack" in str(excinfo.value)
    assert "does not belong to area" in str(excinfo.value)

    valid = serializer.validate({"site": site, "area": area_a, "rack": rack_a})
