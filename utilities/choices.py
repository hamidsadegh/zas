...
from zas.settings import configuration
...
class ChoiceSetMeta(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        if key := attrs.get('key'):
            ...
            if replace_key and replace_key in FIELD_CHOICES:
                # Replace the stock choices
                attrs['CHOICES'] = FIELD_CHOICES[replace_key]
            elif extend_key and extend_key in FIELD_CHOICES:
                # Extend the stock choices
                attrs['CHOICES'].extend(FIELD_CHOICES[extend_key])
